create table instrument (
    ...
    model varchar(100) default null comment '仪器型号',
    status tinyint not null default 1 comment '状态：1启用，0停用',
    create_time bigint not null comment '创建时间',
    update_time bigint not null comment '更新时间',
    primary key(id),
    unique key uk_instrument_code(instrument_code)
) engine=innodb default charset=utf8mb4 comment '仪器基础信息';
...
create table instrument_config (
    ...
    connect_type varchar(20) not null comment '连接方式：SERIAL、TCP_CLIENT、TCP_SERVER',
    port_name varchar(20) default null comment '串口名称',
    ...
    ip varchar(100) default null comment 'TCP地址',
    port int default null comment 'TCP端口',
...
create table instrument_item_mapping (
    ...
    instrument_item_name varchar(100) default null comment '仪器项目名称',
    lis_item_code varchar(50) not null comment 'LIS项目编码',
    lis_item_name varchar(100) default null comment 'LIS项目名称',
    enabled tinyint not null default 1 comment '是否启用：1启用，0停用',
    create_time bigint not null comment '创建时间',
    update_time bigint not null comment '更新时间',
    primary key(id),
...
create table receive_raw_message (
    id bigint not null auto_increment comment '主键ID',
    instrument_id bigint not null comment '仪器ID',
...
) engine=innodb default charset=utf8mb4 comment '接收仪器原始消息队列表';

create table receive_raw_message_large (
    receive_raw_message_id bigint not null comment '接收仪器原始消息队列表Id',
    raw_message longtext not null comment '原始报文',
    primary key(receive_raw_message_id)
) engine=innodb default charset=utf8mb4 comment '接收仪器原始消息队列表扩展表';


create table receive_message_decode (
    id bigint not null auto_increment comment '主键ID',
    receive_raw_message_id bigint not null comment '接收仪器原始消息队列表Id',
    type tinyint not null comment '类型：0检验结果，1查询样本申请信息',
    barcode varchar(50) not null comment '条形码',
    result_json longtext not null comment '解析结果JSON',
    status tinyint not null comment '状态：0待处理，1处理成功，2处理失败',
    create_time bigint not null comment '创建时间',
    update_time bigint not null comment '更新时间',
    primary key(id),
    unique key uk_receive_raw_message_id(receive_raw_message_id)
) engine=innodb default charset=utf8mb4 comment '仪器解析结果表';


create table send_message (
    id bigint not null auto_increment comment '主键ID',
    instrument_id bigint not null comment '仪器ID',
    type tinyint not null comment '类型：0申请单',
    status tinyint not null comment '状态：0待处理，1处理成功，2处理失败',
    create_time bigint not null comment '创建时间',
    update_time bigint not null comment '更新时间',
    primary key(id),
...
create table send_message_large (
    ...
    primary key(receive_raw_message_id)
) engine=innodb default charset=utf8mb4 comment '发送仪器json消息扩展表';
...
create table client_log (
    ...
    level tinyint not null comment '状态(0:普通，1:警告，2:错误)',
